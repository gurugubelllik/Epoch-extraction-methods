function [IDR, MR, FAR, IDA, ERR, IDR_25, Time]=zffeval_new_tel2(foldername,GCI_Method)
%
% Given a foldername, does evaluation for given epoch
% locations
% addpath(genpath(foldername));

% FileList = dir([foldername '*.pcm']);
...
for i=1:1:length(FileList)
    try
        ...
        %         wav_filename=strrep([foldername fname],'GCI_CMU_Telphonic_NEW','CMU_EGGS');
        %         wav_filename=strrep(wav_filename,'pcm','wav')
        %
        %         [egg, fs]=wavread(wav_filename); egg=egg(:,1); egg=resample(egg,8000,fs); fs=8000;
        %
        %
        %         [s]=pcmread([foldername fname]);   % Reading wave file
...
%         polarity=polarity_reskew(speech,fs);
%          
%         speech=speech*polarity;
%         
        polarity=polarity_reskew(degg,fs);
...
%             f0_tmp=f0_tmp(pos);
%             F0mean=mean(f0_tmp);
%             WinLen=round(1500./F0mean);
%             
            [WinLen]=xcorrWinLen(speech,fs) ;   % Window length for trend removal
            [zfSig]=zeroFreqFilter_tel(speech,fs,WinLen,8); % ZFF Signal
...
%                             [WinLen]=xcorrWinLen(speech,fs) ;   % Window length for trend removal
                            ...
                            [zfSig]=zp_zeroFreqFilter_tel(speech,fs,0.98,WinLen,2); % ZFWinLenF Signal filter order 2 4 6 8 10 12
                            ...
                            else if(GCI_Method==7) %%GEFBA%% se_vq
                                    disp(['GEFBA'])
                                    [GOIs, gci, ~, ~, ~, ~, ~]=GEFBA(s,fs);
                                    epochs=s-s;epochs(gci)=1;
                                else if(GCI_Method==8) %CWT
                                        tic
                                        [cwt_ev]=cwt_gci(speech); % ZFWinLenF Signal
                                        [epochs]=epochExtract_cwt(cwt_ev);  % Epoch Extraction
                                        time_elapsed=toc
                                         
                                    else % SPF
                                        [epochs]=SPF_GCI_Extraction(speech,fs);
                                    end
                                end
                            end
                        end
